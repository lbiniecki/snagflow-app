...
If a third party contacts us directly about data concerning them that appears in your VoxSite account, we will direct them to you as the controller.
...
We **never sell** your data. We **never share your inspection data with other customers**.

We do not use your data to train AI models. If we ever introduce features that involve training AI models on customer data, we will notify account administrators in advance and obtain explicit consent before doing so.
...
1. You record a voice note in the app
2. The audio is uploaded to our backend on Railway (servers located in the European Union — Amsterdam, the Netherlands)
3. Our backend forwards the audio to OpenAI's Whisper API for transcription
4. OpenAI returns the text; we store the text on your snag record
5. The audio file itself is retained by us only for the duration needed to transcribe it, then deleted

**International transfer disclosure:** OpenAI processes this audio on infrastructure located in the United States. This constitutes a transfer of personal data outside the European Economic Area (EEA) and the United Kingdom.

We safeguard this transfer through:

- A Data Processing Agreement with OpenAI
- Standard Contractual Clauses (SCCs) approved by the European Commission for EU/EEA data subjects
- The UK International Data Transfer Agreement (IDTA) or the UK Addendum to the EU SCCs for UK data subjects
- OpenAI's processing of audio in line with their published retention policies at the time of processing. As of the last update of this policy, OpenAI states that audio inputs sent via the standard API tier are retained for up to 30 days for abuse monitoring purposes and are not used to train OpenAI models. We do not control OpenAI's policies and you should consult OpenAI's current terms for the most up-to-date position.
...
- Our backend hosting (Railway) is operated in the European Union (Amsterdam, the Netherlands).
- Our database, file storage, and authentication services (Supabase) are operated in the European Union (Dublin, Ireland — `eu-west-1`).
- Our transactional email provider (Resend) routes outbound emails through its EU-West infrastructure (Ireland), but Resend's account-level data (email metadata, delivery logs, API records) is stored in the United States. Email content sent via Resend includes only your email address, the email subject, and the body of transactional notifications (account, billing, report-delivery emails). Your inspection data — projects, snags, photos, voice notes — is **never** sent to Resend.
- Our frontend (Vercel) is delivered through a global edge network. Static page assets may be cached at Vercel edge locations worldwide for performance. Once the app is loaded in your browser, all data interactions go directly to our EU-hosted backend and database.

For non-EU/EEA transfers (Resend account data and OpenAI audio processing in the United States), we rely on Standard Contractual Clauses (EU SCCs) and the UK IDTA / UK Addendum where UK data subjects are concerned.
...
|---|---|---|
| Database, authentication, files, photos | Ireland (`eu-west-1`) | Supabase |
| Backend API servers | Netherlands (Amsterdam) | Railway |
| Frontend / website assets | Global edge network | Vercel |
| Voice transcription | United States | OpenAI |
| Email dispatch routing | Ireland (`eu-west-1`) | Resend |
| Email metadata, logs, API records | United States | Resend |
| Payment processing | Ireland (EU entity) with global processing | Stripe |

All data in transit is encrypted using TLS 1.2 or higher. All data at rest on Supabase is encrypted using AES-256.

**Summary in plain language:**

- Your **inspection data** — the projects, snags, photos, transcribed text, PDF reports — is stored exclusively in the European Union (Ireland and Amsterdam).
- The only routine processing outside the EU is voice audio sent to OpenAI in the United States for transcription, and only when you use the voice-note feature.
- Some operational metadata (transactional email logs at Resend, frontend page caching at Vercel edge nodes, payment processing at Stripe) involves US-based infrastructure. None of this includes your inspection content.

Supabase, Railway, Vercel, Resend, OpenAI, and Stripe are each bound by a Data Processing Agreement with us, and each operates with their own appropriate safeguards for international transfers (Standard Contractual Clauses, the UK IDTA, or the UK Addendum where applicable).
...
| Cancelled account | Deleted within 30 days, except where retention is required for legal, security, or backup purposes |
...
| Voice audio on OpenAI servers | Per OpenAI's retention policy at the time of processing |
| Email logs at Resend | Per Resend's retention schedule (US-stored) |
| Usage logs and crash reports | 90 days |

You may delete individual projects, site visits, and snag items at any time using the app's delete functions. Deleting a project also deletes all associated photos from our storage within a reasonable technical timeframe.

To request earlier deletion of all your personal data, contact sales@voxsite.app. Note that we may be required to retain financial records for the statutory periods above.
...
- **Right to portability** — receive your data in a machine-readable format (we provide JSON export on request; self-service export is in development)
- **Right to object** — object to processing based on legitimate interest
- **Right to withdraw consent** — where we process based on your consent, you can withdraw it at any time
- **Right not to be subject to solely automated decision-making, including profiling** — VoxSite does not make automated decisions or perform profiling that produces legal effects about you

To exercise any right, email sales@voxsite.app. We will respond within 30 days of a verifiable request.
...
| Subprocessor | Purpose | Where data is processed | DPA |
|---|---|---|---|
| Supabase | Database, authentication, file storage | Ireland (`eu-west-1`) | Yes |
| Railway | Backend hosting | Netherlands (Amsterdam) | Yes |
| Vercel | Frontend hosting (global edge) | Global edge network | Yes |
| Stripe | Payment processing | Ireland (EU entity); global processing infrastructure | Yes |
| Resend | Transactional email — dispatch routing in EU-West (Ireland); account data in United States | Ireland + United States | Yes |
| OpenAI | Voice-to-text transcription (only when used) | United States | Yes (with SCCs) |
...
We will update this list if we add or change subprocessors. Material changes will be notified to account administrators by email at least 14 days in advance. Customers may object to a new subprocessor by contacting sales@voxsite.app within the notice period; if the objection cannot be resolved, the customer may terminate the affected subscription as their sole remedy.

## 11. Cookies

VoxSite uses only strictly necessary cookies:
...
Because we only use strictly necessary cookies, consent is generally not required under applicable law. If we ever add non-essential cookies in the future, we will present a consent banner and update this policy.
...
- **Principle of least privilege** — production database access is limited to the owner
...
No system is perfect. If you discover a security issue, please report it responsibly to sales@voxsite.app. We will acknowledge within 48 hours.
...
Your continued use of VoxSite after changes take effect constitutes acceptance of the revised policy. If you do not agree, you may cancel your subscription under Section 6 of our Terms of Service.
